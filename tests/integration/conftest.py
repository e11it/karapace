...
@pytest.fixture(scope="function", name="rest_async_strict")
async def fixture_rest_async_strict(
    request: SubRequest,
    loop: asyncio.AbstractEventLoop,
    kafka_servers: KafkaServers,
    registry_async_client: Client,
) -> AsyncIterator[KafkaRest | None]:
    # Do not start a REST api when the user provided an external service.
    rest_url = request.config.getoption("rest_url")
    if rest_url:
        yield None
        return
    ...
    config.bootstrap_uri = kafka_servers.bootstrap_servers[0]
    config.producer_max_request_size = REST_PRODUCER_MAX_REQUEST_BYTES
    # Enable strict Avro JSON parsing path for integration checks.
...
@pytest.fixture(scope="function", name="rest_async_strict_client")
async def fixture_rest_async_strict_client(
    request: SubRequest,
    loop: asyncio.AbstractEventLoop,
    rest_async_strict: KafkaRest,
    aiohttp_client: AiohttpClient,
) -> AsyncIterator[Client]:
    rest_url = request.config.getoption("rest_url")
    ...
    if rest_url:
        ...
        async def get_client(**kwargs) -> TestClient:
            return await aiohttp_client(rest_async_strict.app)
...
@pytest.fixture(scope="function", name="rest_async_strict_extended_parser")
async def fixture_rest_async_strict_extended_parser(
    request: SubRequest,
    loop: asyncio.AbstractEventLoop,
    kafka_servers: KafkaServers,
    registry_async_client: Client,
...
@pytest.fixture(scope="function", name="rest_async_strict_extended_parser_client")
async def fixture_rest_async_strict_extended_parser_client(
    request: SubRequest,
    loop: asyncio.AbstractEventLoop,
    rest_async_strict_extended_parser: KafkaRest,
    aiohttp_client: AiohttpClient,
) -> AsyncIterator[Client]:
    rest_url = request.config.getoption("rest_url")
    ...
    if rest_url:
        ...
        async def get_client(**kwargs) -> TestClient:
            return await aiohttp_client(rest_async_strict_extended_parser.app)
...
@pytest.fixture(scope="function", name="rest_async_lookup_first_registry_auth")
async def fixture_rest_async_lookup_first_registry_auth(
    request: SubRequest,
    loop: asyncio.AbstractEventLoop,
    kafka_servers: KafkaServers,
    registry_async_client_auth: Client,
) -> AsyncIterator[KafkaRest | None]:
    """REST proxy with lookup-first enabled and read-only Schema Registry credentials.

    The ``reader`` user (see tests/integration/config/karapace.auth.json) only has
    the Read permission on ``Subject:carpet-.*`` resources: schema lookups under
    carpet-* subjects succeed while registering new schemas is denied.
    """
    rest_url = request.config.getoption("rest_url")
    if rest_url:
        yield None
    ...
    config.registry_host = registry.hostname
    config.registry_port = registry.port
    config.registry_user = "reader"
    config.registry_password = "secret"
    config.rest_lookup_schema_before_register = True
    config.waiting_time_before_acting_as_master_ms = 500
    rest = KafkaRest(config=config)
...
@pytest.fixture(scope="function", name="rest_async_lookup_first_registry_auth_client")
async def fixture_rest_async_lookup_first_registry_auth_client(
    request: SubRequest,
    loop: asyncio.AbstractEventLoop,
    rest_async_lookup_first_registry_auth: KafkaRest,
    aiohttp_client: AiohttpClient,
) -> AsyncIterator[Client]:
    rest_url = request.config.getoption("rest_url")
    ...
    if rest_url:
        ...
        async def get_client(**kwargs) -> TestClient:
            return await aiohttp_client(rest_async_lookup_first_registry_auth.app)
