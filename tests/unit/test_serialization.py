...
async def test_upsert_id_for_schema_does_not_reuse_cached_id_from_other_subject(
    ...
    lookup_first_miss_future = asyncio.Future()
    lookup_first_miss_future.set_result(None)
    ...
    lookup_second_miss_future = asyncio.Future()
    lookup_second_miss_future.set_result(None)
    post_second_future = asyncio.Future()
    post_second_future.set_result(schema_id_2)

    mock_registry_client.lookup_schema.side_effect = [
        lookup_first_miss_future,
        lookup_second_miss_future,
    ]
    ...
    serializer = await make_ser_deser(karapace_container, mock_registry_client)
    first_schema_id = await serializer.upsert_id_for_schema(TYPED_AVRO_SCHEMA, subject_1, lookup_first=True)
    second_schema_id = await serializer.upsert_id_for_schema(TYPED_AVRO_SCHEMA, subject_2, lookup_first=True)

    assert first_schema_id == schema_id_1
    assert second_schema_id == schema_id_2
    assert mock_registry_client.method_calls == [
        call.lookup_schema(subject_1, TYPED_AVRO_SCHEMA),
        call.post_new_schema(subject_1, TYPED_AVRO_SCHEMA),
        call.get_schema_for_id(schema_id_1),
        call.lookup_schema(subject_2, TYPED_AVRO_SCHEMA),
