...
.. list-table::
   ...
   * - ``consumer_group_id_prefix``
     - ``karapace-autogenerated-``
     - Prefix for the automatically generated consumer group names used when no
       explicit group id is given (eg. the schema registry's internal consumers).
       Note that any separator (for example a trailing ``-``) must be included in
       the value itself, it is not added automatically.
...
The prefix is configurable with the ``consumer_group_id_prefix`` configuration option.
